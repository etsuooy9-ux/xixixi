...
const initialState = () => ({ saveVersion: SAVE_VERSION, phase: 'home', index: 0, answers: [], finished: false, clues: { archive: false, chat: false, search: false }, ending: null });
let savedState = null;
...
function results() { state.finished = true; state.phase = 'result'; save(); showScreen('result-screen'); $('result-title').textContent = '分析完成。正在生成你的关系报告……'; $('report').classList.add('hidden'); $('ending').classList.add('hidden'); $('anomaly-log').classList.add('hidden'); $('detail-link').classList.add('hidden'); $('restart-button').classList.add('hidden'); setTimeout(() => { $('result-title').textContent = '关系分析报告'; $('report').classList.remove('hidden'); const [warmth, suspicion, independence] = scores(); const values = [Math.min(97, 35 + warmth * 4), Math.min(96, 46 + warmth * 3), Math.min(95, 8 + independence * 4)]; ['dependency', 'closeness', 'independence'].forEach((name, index) => { $(`${name}-score`).textContent = `${values[index]}%`; $(`${name}-bar`).style.width = `${values[index]}%`; }); setTimeout(() => finish(warmth, suspicion, independence), 1800); }, 2100); }
function finish(warmth, suspicion, independence) { const ending = $('ending'); $('control-score').textContent = '无法计算。'; let content; if (state.clues?.archive && state.clues?.chat && state.clues?.search) content = ['Ending 03 · 找到真相', '第四份报告从未被删除。<br>而你并不是第一个完成这份测试的人。', false]; else if (independence >= 29 && suspicion >= 14) content = ['Ending 04 · 他正在看', '你已经回答了这么多问题。<br>现在换我问你一个。', true]; else if (suspicion >= 11 || independence >= 24) content = ['Ending 02 · 发现异常', '你知道这里有问题。<br>可你还没有找到那份被隐藏的记录。', false]; else content = ['Ending 01 · 正常退出', '你的关系报告已经生成。<br><br>感谢使用 Morrow。', false]; state.ending = content[0]; save(); ending.innerHTML = `<h3>${content[0]}</h3><p>${content[1]}</p>`; ending.classList.remove('hidden'); setTimeout(() => { $('anomaly-log').classList.remove('hidden'); $('detail-link').classList.remove('hidden'); }, 1000); if (content[2]) { ending.innerHTML += '<div class="terminal">INPUT REQUIRED_</div><input id="final-input" aria-label="回答他的问题" autocomplete="off" placeholder="输入你的回答" /><button class="quiet-button" id="final-button" type="button">回答</button>'; $('final-button').addEventListener('click', blackout); } $('restart-button').classList.remove('hidden'); }
...
function reset() { clearTimeout(questionTimer); clearTimeout(permissionTimer); permissionShown = false; $('permission-overlay').classList.add('hidden'); $('floating-messages').innerHTML = ''; state = initialState(); state.phase = 'quiz'; localStorage.removeItem(STORAGE_KEY); document.body.className = ''; $('header-status').textContent = '关系档案 · 私密'; renderQuestion(); }
...
function openArchive() { state.clues.archive = true; save(); showScreen('archive-screen'); }
function showSearchResults() { const key = $('archive-search').value.trim().toLowerCase(); const results = { morrow: 'Morrow / 企业介绍：匿名关系洞察服务。', '04': '档案 MR-04-██71：第四份报告已被标记为删除。', '报告': '内部备注：不要让她看到第四份报告。', '用户': '用户记录：第二用户曾在 23:18 登录。', '删除': '删除请求失败。源文件仍保留在内部记录中。', '她': '她完成测试后，不会记得自己看过什么。', '昨天': '昨日登录：23:18 · IP 已被打码。' }; const value = Object.entries(results).find(([term]) => key.includes(term)); if (value) { state.clues.search = true; save(); $('search-results').innerHTML = `<p><b>检索结果 / 1</b></p><p>${value[1]}</p>`; } else $('search-results').innerHTML = '<p>未找到公开记录。</p>'; }
...
$('detail-link').addEventListener('click', openArchive); $('archive-back').addEventListener('click', results); $('archive-search').addEventListener('input', showSearchResults); $('load-chat').addEventListener('click', () => { state.clues.chat = true; save(); $('chat-messages').innerHTML += '<p class="chat-a">A：如果她看到第四份报告呢？</p><p class="chat-b">B：那就只能让系统替她忘记。</p><p class="chat-a">A：她正在搜索。</p>'; $('load-chat').disabled = true; $('load-chat').textContent = '记录已加载'; });
